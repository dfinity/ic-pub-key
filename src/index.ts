...
const secp256k1 = program
    .command('secp256k1')
    .description('Operations related to secp256k1 keys');
...
secp256k1
    ...
    .requiredOption('--path <derivationpath>', 'The derivation path')
    .action((options) => {
        let pubkey_with_chain_code = Secp256k1PublicKeyWithChainCode.fromBlob(options.pubkey, options.chaincode);
        let parsed_derivationpath = DerivationPath.fromBlob(options.path);
