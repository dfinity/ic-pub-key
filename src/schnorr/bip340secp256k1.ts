...
export class PublicKeyWithChainCode {
	 ...
	 */
	constructor(
		public readonly public_key: curve.ProjectivePoint,
	...
	static fromArray(public_key_array: number[], chain_code_array: number[]): PublicKeyWithChainCode {
		...
		return new PublicKeyWithChainCode(
			curve.ProjectivePoint.fromHex(public_key_hex),
	...
	static fromHex(public_key_hex: string, chain_code_hex: string): PublicKeyWithChainCode {
		let public_key = curve.ProjectivePoint.fromHex(public_key_hex);
...
export class DerivationPath {
	 ...
	 */
	derive_offset(
		pt: curve.ProjectivePoint,
		chain_code: ChainCode
	): [curve.ProjectivePoint, bigint, ChainCode] {
		// Deep copy the point: (Is there a better way?)
		pt = new curve.ProjectivePoint(pt.px, pt.py, pt.pz);
		...
		for (let idx of this.path) {
            ...
            */
			pt = pt.add(curve.ProjectivePoint.BASE.mul(offset));
			sum += offset;
	...
	static ckd_pub(
		...
		pt: curve.ProjectivePoint,
		chain_code: ChainCode
	): [ChainCode, bigint, curve.ProjectivePoint] {
		/*
