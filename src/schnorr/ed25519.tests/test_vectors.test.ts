...
import { ChainCode, DerivationPath, PublicKey, PublicKeyWithChainCode } from '../ed25519';
...
describe('Test Vectors', () => {
	...
	testVectors.schnorr.ed25519.test_vectors.forEach((vector) => {
		it(`should derive correct key for test vector ${vector.name}`, () => {
			// Create the input key with chain code
			const public_key = PublicKey.fromHex(vector.public_key);
